...
(function() {
  ...
  while (input != commands[3]) {
    ...
    input = prompt("入力してください"); //   - whileループの中でprompt関数を使って、値を入力できるようにして、入力された内容は変数「input」に格納する
    //     - prompt: https://developer.mozilla.org/ja/docs/Web/API/Window/prompt
    //   - 変数inputの値によって、次の機能を実行する
    if (input == commands[0]) {
      showTodos(); //     1. 「確認」と入力された時: showTodos関数を実行する
    } else if (input == commands[1]) {
      //     2. 「追加」と入力された時: createTodo関数を実行する
      createTodo(); //     3. 「削除」と入力された時: deleteTodo関数を実行する
    } else if (input == commands[2]) {
      //     4. 「終了」と入力された時: Consoleに'終了します'と表示して、Whileループを終了    ()する
      deleatTodo();
  ...
  function deleatTodo() {
    ...
    // console.log(todos.splice(input, 1));
    let deleatedTodos = todos.splice(input, 1);
    alert(deleatedTodos + "を削除しました");
    showTodos();
  }
})();
